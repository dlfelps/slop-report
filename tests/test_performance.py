...
def _make_worktree_mock(returncode: int = 0):
...
def test_run_head_timeout():
    with patch("src.metrics.performance._run_pytest", return_value=({}, True)):
        with patch("subprocess.run", return_value=_make_worktree_mock()):
...
def test_run_worktree_fails_returns_na():
    """When worktree add fails, report N/A rather than crashing."""
    with patch("src.metrics.performance._run_pytest", return_value=(SAMPLE_TIMINGS, False)):
        with patch("subprocess.run", return_value=_make_worktree_mock(returncode=1)):
            with patch("tempfile.mkdtemp", return_value="/tmp/fake"):
